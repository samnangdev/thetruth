                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="edit.php?id=<?php echo $row['ID']; ?>"
                                    class="<?php echo ($_SESSION['user_type_name'] === 'Author') ? 'disabled-link' : ''; ?>">
                                    <i class="bx bxs-edit"></i>
                                </a>
                                &nbsp;
                                <a href="javascript:void(0);"
                                    class="btnDelete <?php echo ($_SESSION['user_type_name'] === 'Author') ? 'disabled-link' : ''; ?>"
                                    data-id="<?php echo $row['ID']; ?>"
                                    name="btnDelete"
    ...
    document.addEventListener("DOMContentLoaded", function() {
        document.querySelectorAll(".toggle-status").forEach(function(checkbox) {
            checkbox.addEventListener("change", function() {
                ...
                window.location.href = `<?php echo BASE_URL ?>controllers/TagController.php?update_id=${id}&status=${newStatus}`;
            });
        });
    });
</script>
